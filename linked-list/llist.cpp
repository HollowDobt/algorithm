...
class LinkedList {
   private:
    ...
    struct Iterator {
       ...
       public:
        // 构造函数
        Iterator(Node* ptr, Node* head, Node* foot)
            : _ptr(ptr), _head(head), _foot(foot) {}
...
template <typename T>
T const& LinkedList<T>::Node::val() const {
    return this->_value;
}
template <typename T>
T& LinkedList<T>::Node::val() {
    return this->_value;
}

template <typename T>
LinkedList<T>::Node* const& LinkedList<T>::Node::next() const {
    return this->_next;
}
template <typename T>
LinkedList<T>::Node*& LinkedList<T>::Node::next() {
    return this->_next;
}

template <typename T>
LinkedList<T>::Node* const& LinkedList<T>::Node::prev() const {
    return this->_prev;
}
template <typename T>
LinkedList<T>::Node*& LinkedList<T>::Node::prev() {
    return this->_prev;
}
...
// 该函数使用前必须确保值有意义
template <typename T>
T const& LinkedList<T>::Iterator::operator*() const {
...
bool LinkedList<T>::Iterator::operator==(
    LinkedList<T>::Iterator const& rhs) const {
...
LinkedList<T>::LinkedList(size_t size)
    : _head(new Node()), _foot(new Node()), _size(size) {
    Node* prev = _head;
    while (size--) {
        Node* next = new Node();
...
template <typename T>
LinkedList<T>::LinkedList(size_t size, T const& val)
    : _head(new Node()), _foot(new Node()), _size(size) {
    Node* prev = _head;
    while (size--) {
...
void LinkedList<T>::push_front(T const& val) {
    ...
    _head->next() = ptr, ptr->next() = temp, ptr->prev() = _head,
    temp->prev() = ptr;
    _size++;
}
...
void LinkedList<T>::push_back(T const& val) {
    ...
    _foot->prev() = ptr, ptr->prev() = temp, ptr->next() = _foot,
    temp->next() = ptr;
    _size++;
}
...
void LinkedList<T>::insert(size_t index, T const& val) {
    ...
    ltemp->next() = new_node, new_node->prev() = ltemp, new_node->next() = temp,
    temp->prev() = new_node;
    _size++;
}
